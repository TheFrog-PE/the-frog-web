...
/* ========================================================
       8. ASISTENTE DE VOZ INTELIGENTE (THE FROG AI)
       ======================================================== */
...
if (!SpeechRecognition || !synth) {
    ...
    console.warn("Tu navegador no soporta el asistente de voz.");
} else {
    ...
    // --- EL TRUCO PARA EL SALUDO INICIAL ---
    // Se activará al primer clic o toque en CUALQUIER parte de la página
    function initialGreeting() {
        if (!hasGreeted) {
            hasGreeted = true;
            speak("Bienvenido a The Frog Soluciones Digitales. Soy tu asistente virtual. Toca el botón del micrófono en la esquina inferior derecha para conversar conmigo y conocer más sobre nuestros servicios.");

            // Quitamos el evento para que no vuelva a saludar al hacer clic en otras cosas
            document.removeEventListener('click', initialGreeting);
            document.removeEventListener('touchstart', initialGreeting);
        }
    }
    document.addEventListener('click', initialGreeting);
    document.addEventListener('touchstart', initialGreeting);


    // --- EL MINI CEREBRO CONVERSACIONAL ---
    const knowledgeBase = [
        {
            keywords: ['hola', 'buenos días', 'buenas tardes', 'buenas noches', 'qué tal', 'saludos'],
            response: "¡Hola! Qué gusto saludarte. Estoy aquí para ayudarte. Puedes preguntarme sobre nuestros planes, precios o servicios de diseño de esta página."
        },
        {
            keywords: ['tengo una duda', 'una pregunta', 'ayuda', 'necesito saber', 'información', 'dime algo'],
            response: "¡Claro que sí! ¿Cuál es tu duda? Puedo explicarte cómo funcionan nuestros paquetes integrales, las webs corporativas o las promociones vigentes."
        },
        {
            keywords: ['planes', 'paquetes', 'precios', 'costo', 'cuánto', 'cuanto'],
            response: "Tenemos tres tipos de paquetes. Los paquetes Integrales que van desde los 510 soles al mes con el descuento actual. Los paquetes Plus desde 765 soles. Y soluciones de solo tecnología, como Landing Pages y Webs Corporativas de pago único. ¿Te interesa alguno en particular?"
        },
        {
            keywords: ['básico', 'basico'],
            response: "El plan básico incluye la gestión de 2 redes sociales, 8 publicaciones mensuales, respuestas a comentarios y reporte. Su precio con descuento es de 510 soles al mes."
        },
        {
            keywords: ['intermedio'],
            response: "Nuestro plan recomendado es el Intermedio. Incluye 3 redes sociales, 12 posts, 4 videos cortos y gestión de pauta. Con el descuento te sale a 850 soles mensuales."
        },
        {
            keywords: ['avanzado'],
            response: "El plan Avanzado es una estrategia 360 con 20 contenidos, sesión audiovisual y moderación intensiva. Su precio con promoción es de 1530 soles mensuales."
        },
        {
            keywords: ['tecnología', 'web', 'landing', 'sistema', 'medida', 'tecnologia'],
            response: "En tecnología ofrecemos Landing Pages por 765 soles, Webs Corporativas por 1275 soles, ambos de pago único con 3 meses de garantía. También hacemos sistemas a medida previa evaluación."
        },
        {
            keywords: ['diseño', 'logo', 'identidad'],
            response: "Contamos con diseño de Logo Básico por 350 soles, y una Identidad Visual Completa por 900 soles, ambos de pago único."
        },
        {
            keywords: ['hosting', 'dominio', 'renovación', 'anual'],
            response: "Sí, nuestros servicios de desarrollo web incluyen el primer año de Hosting y Dominio. Después del primer año, solo deberás abonar la renovación anual del servidor."
        },
        {
            keywords: ['anuncios', 'publicidad', 'ads', 'presupuesto'],
            response: "El presupuesto para anuncios de Facebook o Google no está incluido en el precio de nuestros planes. Nosotros cubrimos la gestión profesional de las campañas."
        },
        {
            keywords: ['descuento', 'oferta', 'promoción', 'promocion'],
            response: "Actualmente tenemos un 15% de descuento durante los primeros 3 meses en la mayoría de nuestros servicios por ser tu primera vez con nosotros."
        }
    ];
    ...
    function speak(text) {
        ...
        utterance.onstart = () => {
            isSpeaking = true;
            if (voiceBtn) {
                voiceBtn.classList.replace('bg-accent2', 'bg-blue-500');
                voiceBtn.classList.replace('bg-red-500', 'bg-blue-500');
                voiceBtn.classList.add('animate-bounce');
            }
        };
        ...
        utterance.onend = () => {
            isSpeaking = false;
            if (voiceBtn) {
                voiceBtn.classList.replace('bg-blue-500', 'bg-accent2');
                voiceBtn.classList.remove('animate-bounce');
            }
        };
    ...
    function processUserSpeech(transcript) {
        ...
        if (foundResponse) {
            ...
            speak("Lo siento, como asistente virtual solo puedo brindarte información sobre los servicios, planes, precios y preguntas frecuentes de la agencia The Frog. ¿Te gustaría saber sobre nuestros paquetes de marketing o diseño?");
        }
    }
    ...
    recognition.onstart = () => {
        isListening = true;
        if (voiceBtn) {
            voiceBtn.classList.replace('bg-accent2', 'bg-red-500');
    ...
    // --- MANEJO DE ERRORES DEL MICRÓFONO MEJORADO ---
    recognition.onerror = (event) => {
        console.error("Error del micrófono:", event.error);
        isListening = false;
        if (voiceBtn) {
            voiceBtn.classList.replace('bg-red-500', 'bg-accent2');
            voiceBtn.classList.remove('animate-pulse');
        }

        if (event.error === 'not-allowed') {
            speak("Parece que no tengo permiso para usar el micrófono. Por favor, habilítalo en la configuración de tu navegador para poder conversar.");
        } else if (event.error === 'no-speech') {
            speak("No pude escucharte bien. ¿Puedes repetirlo, por favor?");
        }
    };

    recognition.onend = () => {
        isListening = false;
        if (!isSpeaking && voiceBtn) {
            voiceBtn.classList.replace('bg-red-500', 'bg-accent2');
            voiceBtn.classList.remove('animate-pulse');
        }
    };
    ...
    if (voiceBtn) {
        voiceBtn.addEventListener('click', (e) => {
            // Evitamos que el clic en el botón dispare el saludo global si aún no ha saludado
            e.stopPropagation();
            ...
            if (!hasGreeted) {
                hasGreeted = true;
                // Quitamos los listeners globales porque ya saludó al hacer clic en el micro
                document.removeEventListener('click', initialGreeting);
                document.removeEventListener('touchstart', initialGreeting);
                speak("¡Hola! Soy el asistente de The Frog. Toca el micrófono de nuevo y dime, ¿En qué te puedo ayudar hoy?");
            } else {
                if (!isListening) {
                    try {
                        recognition.start();
                    } catch (error) {
                        console.error("El reconocimiento ya estaba iniciado.", error);
                    }
                }
            }
        });
